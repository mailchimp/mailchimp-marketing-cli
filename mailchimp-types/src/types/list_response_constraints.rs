...
pub struct ListResponseConstraints {
...
impl ListResponseConstraints {
    pub fn builder() -> ListResponseConstraintsBuilder {
        <ListResponseConstraintsBuilder as Default>::default()
    }
}

#[derive(Clone, PartialEq, Default, Debug)]
#[non_exhaustive]
pub struct ListResponseConstraintsBuilder {
    current_total_instances: Option<i64>,
    max_instances: Option<i64>,
    may_create: Option<bool>,
}

impl ListResponseConstraintsBuilder {
    ...
    /// Consumes the builder and constructs a [`ListResponseConstraints`].
    /// This method will fail if any of the following fields are not set:
    /// - [`max_instances`](ListResponseConstraintsBuilder::max_instances)
    /// - [`may_create`](ListResponseConstraintsBuilder::may_create)
    pub fn build(self) -> Result<ListResponseConstraints, BuildError> {
        Ok(ListResponseConstraints {
            current_total_instances: self.current_total_instances,
            max_instances: self.max_instances.ok_or_else(|| BuildError::missing_field("max_instances"))?,
            may_create: self.may_create.ok_or_else(|| BuildError::missing_field("may_create"))?,
        })
    }
}
